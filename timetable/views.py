...
def aao_logins(request):
    ...
    if request.method == 'POST':
        ...
        pawd = models.user_info.objects.get(user=stuID).pwd
        pwd1 = request.get_signed_cookie('pwd', salt=pawd, default='')
        if is_login and pwd1:
...
def login_requires(func):
    ...
    def inner(request, *args, **kwargs):
        ...
        is_login = request.get_signed_cookie('is_login', salt='sha1', default='')
        stuID = request.get_signed_cookie('ID', salt='sha1', default='')
        pawd = models.user_info.objects.get(user=stuID).pwd
        print(pawd)
        pwd1 = request.get_signed_cookie('pwd', salt=str(pawd), default='')
        if not(is_login and pwd1):
